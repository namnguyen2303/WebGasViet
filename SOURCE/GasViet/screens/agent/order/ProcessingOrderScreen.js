...
class ProcessingOrderScreen extends Component {
    constructor(props) {
        super(props);
        this.state = {
            token: '',
    ...
    render() {
        ...
        // reactotron.log(listOrderState.data, "test data đầu");
        ...
        return (
            <View style={[theme.styles.containter]}>
                <FlatList
                    style={{ width: '100%', alignSelf: 'center', flexGrow: 1, backgroundColor: '#F9F9F9', paddingTop: Platform.OS == 'android' ? 30 : 10 }}
    ...
    renderItem = ({ item, index }) => {
        return (
            <OrderItem key={index} item={item} type_order={1} />
        )
    }
}
...
const mapStateToProps = (state) => ({
    listOrderState: state.listOrderReducer.processing,
})
...
export default connect(mapStateToProps, mapDispatchToProps)(ProcessingOrderScreen)
