...
class NotifyScreen extends Component {
  ...
  componentDidMount() {
    ...
    if (this.state.data) {
      ...
      if (this.state.data.orderID) {
        // reactotron.log(this.state.data.orderID, "log orderId in noti");
        this.orderDetail();
      }
    }
  }
  ...
  orderDetail = async () => {
    ...
    try {
      const res = await API.getOrderDetail(this.state.data
        ? this.state.data.orderID
        : item.orderID);
      // reactotron.log("true");
      await this.setState({
    ...
    return (
      <View style={{ flex: 1 }}>
        <ScrollView style={{ flex: 1 }}>
          ...
          {this.state.type == ORDER_TYPE.processing &&
            this.renderBtnCall(item.buyerPhone)}
